...
def model(x, is_train, reuse):
    with tf.variable_scope("STN", reuse=reuse):
        ...
        nt = Conv2d(nt, 16, (3, 3), (2, 2), act=tf.nn.relu, padding='SAME', name='tc1')
        #32x32x16
        ...
        # 1X1X64
        nt = Conv2d(nt, 3, (1, 1), (1, 1),act=None, padding='SAME', name='tc7')
        # 1X1X1
...
if(train==1):
    tl.files.save_npz(net_train.all_params, name='model_3accuracy.npz')
...
if(train==0):
    load_params = tl.files.load_npz(path='', name='model.npz')
    tl.files.assign_params(sess, load_params, net_train)
